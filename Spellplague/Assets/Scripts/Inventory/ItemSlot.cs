...
namespace Spellplague.Inventory
{
    public class ItemSlot : MonoBehaviour, IPointerClickHandler, IPointerDownHandler, IBeginDragHandler, IDragHandler, IPointerUpHandler
    {
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (CheckForInBounds(pointerPositionRelativeToBackground))
            {
                localSlotTransform.position = eventData.position;
            }
        }

        private bool CheckForInBounds(Vector2 pointerPosRelativeToBackground)
        {
        ...
        private void OnTriggerEnter2D(Collider2D collision) => ToggleTouchBool(collision, isTouching: true);

        private void OnTriggerExit2D(Collider2D collision) => ToggleTouchBool(collision, isTouching: false);
